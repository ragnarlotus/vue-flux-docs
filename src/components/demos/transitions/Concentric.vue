<script setup lang="ts">
	...
	import { Concentric } from 'vue-flux';
...
const transitions = shallowReactive([
	{
		component: Concentric,
		options: shallowReactive({
			circles: ${transitions[0].options.circles},
...
<template>
	<div class="demo">
		...
		<label>
			<span>Circles:</span>
			<Slider
				v-model="transitions[0].options.circles"
				:min="4"
				:max="20"
				@slide="(newCircles: number) => (transitions[0].options.circles = newCircles)"
			/>
		</label>
